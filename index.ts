...
while ((!correct_username.test(x))) {
  ...
  if (
    correct_username.test(x)
    
  )
   {
  } else {
    console.log(
      "Please enter valid username (username cannot be empty and space cannot be used in username)"
    );
  }
}
  ...
  if (correct_pass.test(y)) {
    ...
    while (!correct_pass.test(y)) {
      if (pass_try == 1) {
        pass_try -= 1;
        console.log(
          `You have entered invalid password 3 times. Your account have be blocked. Please contact your bank`
        );
    ...
    if (a != "Balance Inquiry") {
      ...
      if (b == "No") {
        ...
        if (current_balance < parseInt(a)) {
          ...
          console.log(
            `Your remaining balance is ${current_balance - parseInt(a)}`
          );
          console.log(`Thankyou for using our ATM`);
        }
      }
